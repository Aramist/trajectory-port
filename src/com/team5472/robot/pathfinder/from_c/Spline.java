...
public class Spline {
    ...
    public static Coord interpolatedCoords(Spline s, double percentage){
        ...
        return new Coord(
        x * cos_theta - y * sin_theta + s.xOffset,
        x * sin_theta + y * cos_theta + s.yOffset);
    }

    public static double deriv(Spline s, double percentage){
        double x = percentage * s.knotDistance;
        return (5*s.a*x + 4*s.b) * (x*x*x) + (3*s.c*x + 2*s.d) * x + s.e;
    }

    public static double deriv2(double a, double b, double c, double d, double e, double k, double p){
        double x = p * k;
        return (5*a*x + 4*b) * (x*x*x) + (3*c*x + 2*d) * x + e;
    }

    public static double angle(Spline s, double percentage){
        return MathUtils.boundRadians(Math.atan(deriv(s, percentage)) + s.angleOffset);
    }

    public static double distance(Spline s, int sampleCount) {
        ...
        double deriv0 = deriv2(a, b, c, d, e, knot, 0);
    ...
    public static double progressForDistance(Spline s, double distance, int sample_count){
